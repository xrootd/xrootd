...
readonly XROOTD_SRC="localhost:8468"
readonly XROOTD_DST="localhost:8469"
...
setup() {
	...
	PORT=${XROOTD_SRC##*:} launch_xrootd https.cfg xrootd_src
	PORT=${XROOTD_DST##*:} launch_xrootd https.cfg xrootd_dst
...
@test "pull copy sends the Source header to the destination server" {
	run -0 xrdcp -T only https://${XROOTD_SRC}//file_src https://${XROOTD_DST}//file_dst
	run -0 grep "Source: https://${XROOTD_SRC}//file_src" xrootd_dst.log
}

@test "pull copy without --force fails when the destination exists" {
	run ! xrdcp -T only https://${XROOTD_SRC}//file_src https://${XROOTD_DST}//file_dst_overwrite
}
...
@test "push copy sends the Destination header to the source server" {
	run -0 xrdcp -T push only https://${XROOTD_SRC}//file_src https://${XROOTD_DST}//file_dst
	run -0 grep "Destination: https://${XROOTD_DST}//file_dst" xrootd_src.log
}
...
@test "push copy with --force succeeds when the destination exists" {
	run -0 xrdcp --force -T push only https://${XROOTD_SRC}//file_src https://${XROOTD_DST}//file_dst_overwrite
}
