...
//
// XrdOfsStats now exposes its counters through the XrdMetrics registry (the
// constructor registers readers over its Data fields) and reproduces the legacy
// <stats id="ofs"> wire format from a registry snapshot. This sets each field to
// a distinct value (in the order Report() emits them) and pins the exact XML,
// exercising the full register -> snapshot -> render round-trip.
//------------------------------------------------------------------------------
...
TEST(XrdOfsStats, SummaryXmlIsByteStable)
{
  ...
  stats.Data.numOpenR    = 1;
  stats.Data.numOpenW    = 2;
  stats.Data.numOpenP    = 3;
  stats.Data.numUnpsist  = 4;
