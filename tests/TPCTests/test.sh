...
# The HTTP status code is not enough: the destination server answers 202 as soon
...
for idx in "${!multistream_suffixes[@]}"; do
    ...
    assert_eq "202" "$(perform_http_tpc "$src_idx" "$dst_idx" "pull" "$BEARER_TOKEN" "$BEARER_TOKEN" "${suffix}" "${MULTISTREAM_STREAMS}")" \
