...
@test "Tape REST reads a body spanning TLS records and the HTTP buffer" {
  local body="$BATS_TEST_TMPDIR/large.json"
  # Valid JSON with padding larger than the protocol's 1 MiB buffer.
  printf '%1048600s' '' > "$body"
  printf '{"paths":["%s"]}' "$TAPE_FILE_PATH" >> "$body"

  run curl --silent --show-error --max-time 30 --cacert "$X509_CA_FILE" \
    --header "Authorization: Bearer $(<"$READ_TOKEN")" \
    --header "Content-Type: application/json" --header 'Expect: 100-continue' \
...
@test "Tape REST reauthorizes ID-only operations on the same TLS connection" {
  run "$XRDFS_BIN" prepare -s "$TAPE_FILE_URL"
  assert_success
  local request_id="$output"
  local url="$ORIGIN_URL/api/v1/stage/$request_id"

  run curl --silent --show-error --max-time 30 --cacert "$X509_CA_FILE" \
    --header "Authorization: Bearer $(<"$READ_TOKEN")" \
    --output /dev/null --write-out '%{http_code}:%{num_connects}\n' "$url" \
    --next --silent --show-error --max-time 30 --cacert "$X509_CA_FILE" \
    --output /dev/null --write-out '%{http_code}:%{num_connects}\n' "$url" \
    --next --silent --show-error --max-time 30 --cacert "$X509_CA_FILE" \
    --header "Authorization: Bearer $(<"$READ_TOKEN")" \
    --output /dev/null --write-out '%{http_code}:%{num_connects}\n' "$url"
