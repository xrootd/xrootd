...
if(COVERAGE)
  string(APPEND CTEST_BUILD_NAME " [Coverage]")
  find_program(CTEST_COVERAGE_COMMAND NAMES gcov)
  if(NOT CTEST_COVERAGE_COMMAND)
    message(FATAL_ERROR "Could not find gcov command")
  endif()
...
if(MEMCHECK)
  ...
  string(APPEND CTEST_BUILD_NAME " [Valgrind]")
...
if(STATIC_ANALYSIS)
  string(APPEND CTEST_BUILD_NAME " [ClangTidy]")
...
list(PREPEND CMAKE_ARGS -DCMAKE_C_FLAGS=${CMAKE_C_FLAGS})
list(PREPEND CMAKE_ARGS -DCMAKE_CXX_FLAGS=${CMAKE_CXX_FLAGS})
...
if(DEFINED ENV{GITHUB_ACTIONS})
  ...
  if("$ENV{GITHUB_EVENT_NAME}" MATCHES "pull_request")
    ...
    string(REGEX REPLACE "/merge" "" CTEST_CHANGE_ID "$ENV{GITHUB_REF_NAME}")
    string(APPEND CTEST_BUILD_NAME " ($ENV{GITHUB_ACTOR}) #${CTEST_CHANGE_ID}")
  else()
...
if(${CONFIG_RESULT} EQUAL 0)
  section("Build")
  ctest_build(RETURN_VALUE BUILD_RESULT NUMBER_WARNINGS WARNINGS)
  endsection()
  if(WARNINGS GREATER 0)
    message(SEND_ERROR "Build contains compiler warnigns")
  endif()
else()
  message(SEND_ERROR "Configuration failed")
endif()
...
if(NOT ${BUILD_RESULT} EQUAL 0)
  ...
  if(NOT DEFINED CTEST_MEMORYCHECK_TYPE)
    ...
    section("${CTEST_MEMORYCHECK_TYPE}")
    ctest_memcheck(RETURN_VALUE TEST_RESULT
      PARALLEL_LEVEL $ENV{CTEST_PARALLEL_LEVEL})
    endsection()
  endif()
