...
int TapeApiHandler::SendError(XrdHttpExtReq &req, int code,
                              const std::string &message, bool close,
                              const std::string &allow)
{
  std::string headers = "Content-Type: application/problem+json";
  if (!allow.empty()) headers += "\r\nAllow: " + allow;
  const std::string body = Json({{"status", code}, {"title", message}}).dump();
  if (close) {
    if (req.StartSimpleResp(code, nullptr, headers.c_str(),
                            body.size(), false) >= 0 && req.verb != "HEAD")
      req.SendData(body.data(), body.size());
    // Unconsumed body bytes must never become the next HTTP request.
    return -1;
  }
  if (req.verb == "HEAD")
    return req.StartSimpleResp(code, nullptr, headers.c_str(),
                               body.size(), true);
  return req.SendSimpleResp(code, nullptr,
    headers.c_str(), body.c_str(), body.size());
}
                           ...
                           bool stage, bool archive)
{
  ...
  const int rc = req.RunNative(native, payload,
    [stage, archive, operation](XrdHttpExtReq &request, const XrdHttpExtReq::NativeResponse &result) {
      ...
      if (operation == kXR_query) {
        // The JSON lexer treats NUL as EOF. Reject hidden trailing content
        // after removing the native protocol's terminal string padding.
...
{
  body.clear();
  if (XrdOucTUtils::caseInsensitiveFind(req.headers, "transfer-encoding")
      != req.headers.end()) {
    errorCode = 411;
    error = "Tape REST requires Content-Length; chunked bodies are unsupported";
    return false;
  }
  ...
  const auto expect = XrdOucTUtils::caseInsensitiveFind(req.headers, "expect");
  if (expect != req.headers.end() && !strcasecmp(expect->second.c_str(), "100-continue")) {
    const std::string response = "HTTP/1.1 100 Continue\r\n\r\n";
    if (req.SendData(response.data(), response.size()) < 0) {
      errorCode = 400;
      error = "could not acknowledge request body";
      return false;
    }
  }

  // Each call returns a borrowed contiguous view, which may end at a TLS
  // record or the protocol buffer boundary. Copy it before the next read.
  body.reserve(req.length);
  while (body.size() < static_cast<size_t>(req.length)) {
    char *buffer = nullptr;
    const int wanted = std::min<size_t>(64 * 1024, req.length - body.size());
    const int bytesRead = req.BuffgetData(wanted, &buffer, true);
    if (bytesRead <= 0 || bytesRead > wanted || buffer == nullptr) {
      errorCode = 400;
...
{
  if (body.find('\0') != std::string::npos) {
    error = "malformed JSON request: raw NUL byte";
    return false;
  }
  try
  {
...
int TapeApiHandler::ProcessReq(XrdHttpExtReq &req)
{
  ...
  std::string body;
  {
    ...
    {
      return SendError(req, errorCode, error, true);
    }
  }
...
{
  if(req.verb != "GET") return SendError(req, 405, "expected GET", false, "GET");
...
{
  if(req.verb != "POST") return SendError(req, 405, "expected POST", false, "POST");
  Json json;
  std::string error;
...
int TapeApiHandler::StageStatus(XrdHttpExtReq &req,
                                const std::string &requestId)
{
  if(req.verb != "GET") return SendError(req, 405, "expected GET or DELETE", false, "GET, DELETE");
...
int TapeApiHandler::StageDelete(XrdHttpExtReq &req,
                                const std::string &requestId)
{
  if(req.verb != "DELETE") return SendError(req, 405, "expected GET or DELETE", false, "GET, DELETE");
  if (!XrdOfsPrepProtocol::IsId(requestId)) return SendError(req, 404, "unknown prepare request");
