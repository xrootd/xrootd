...
//
   X509_STORE *newX509 = SSL_CTX_get_cert_store(xrdctxnew->pImpl->ctx);

#if OPENSSL_VERSION_NUMBER < 0x10101000L
   SSL_CTX_set1_verify_cert_store(xrdctxnew->pImpl->ctx, newX509);
   SSL_CTX_set1_chain_cert_store(xrdctxnew->pImpl->ctx, newX509);
#else
   SSL_CTX_set1_cert_store(xrdctxnew->pImpl->ctx, newX509);
#endif
...
// OK, now we can drop all the locks
