...
struct XrdTlsContextImpl
{
    ...
    ~XrdTlsContextImpl() {
        if (ctx)     {
            {
                XrdSysRWLockHelper locker(XrdTlsContext::SSLMapsMutex);
                auto ctxCountItor = XrdTlsContext::SSLCTXCount.find(ctx);
                if(ctxCountItor != XrdTlsContext::SSLCTXCount.end()) {
                    if(ctxCountItor->second == 0) {
                        //The ctx is not used by any transfer, we can free it
                        //If it is used, the transfer will free it when CleanupSession() is called
                        SSL_CTX_free(ctx);
                        XrdTlsContext::SSLCTXCount.erase(ctx);
                    }
                }
            }
        }
...
std::unordered_map<SSL*,SSL_CTX*> XrdTlsContext::SSLtoSSLCTX;
std::unordered_map<SSL_CTX *,uint64_t> XrdTlsContext::SSLCTXCount;
XrdSysRWLock XrdTlsContext::SSLMapsMutex;
...
//
   ctx_tracker.Keep();
   //This newly created context is not used yet by any SSL* structure, initialize its usage
   //count to 0 as it is not yet used by any transfer
   {
       XrdSysRWLockHelper lock(SSLMapsMutex,0);
       SSLCTXCount[pImpl->ctx] = 0;
   }
}
...
//
   ...
   if (!(pImpl->ctxnew)) {
       ...
       {
           //A new SSL structure is created with the pImpl->ctx,
           //Add this new pair to the tracking maps
           XrdSysRWLockHelper lock(SSLMapsMutex, 0);
           SSLtoSSLCTX[ssl] = pImpl->ctx;
           SSLCTXCount[pImpl->ctx]++;
...
//
   ...
   //The new content has been generated by the Refresh thread. So we must use it
   XrdTlsContext *xrdctxnew = pImpl->ctxnew;

// Generate a new session (might as well to keep the lock we have)
// This session comes from the context generated by the Refresh thread
//
   ssl = SSL_new(xrdctxnew->pImpl->ctx);
   {
        //A new SSL structure is created with the newly created xrdtlscontext->pImpl->ctx,
        //Add this new pair to the tracking maps
        XrdSysRWLockHelper lock(SSLMapsMutex, 0);
        SSLtoSSLCTX[ssl] = xrdctxnew->pImpl->ctx;
        SSLCTXCount[xrdctxnew->pImpl->ctx]++;
        //Check if there are active transfers for the current pImpl->ctx
        //If there are not, it should be freed before being re-assigned
        //with the ctx coming from the xrdctxnew Context
        if(SSLCTXCount[pImpl->ctx] == 0) {
            SSL_CTX_free(pImpl->ctx);
            SSLCTXCount.erase(pImpl->ctx);
        }
   }

    // OK, now assign the newly created SSL context to the current pImpl
    // and we can drop all the locks
    pImpl->ctx = xrdctxnew->pImpl->ctx;
    xrdctxnew->pImpl->ctx = 0;
    pImpl->ctxnew = nullptr;
    pImpl->crlMutex.UnLock();
    return ssl;
...
void XrdTlsContext::CleanupSession(SSL *ssl) {
    XrdSysRWLockHelper lock(SSLMapsMutex, 0);
    auto ctxItor = SSLtoSSLCTX.find(ssl);
    if(ctxItor != SSLtoSSLCTX.end()) {
        SSL_CTX * ctxTransfer = ctxItor->second;
        SSLtoSSLCTX.erase(ctxItor);
        SSLCTXCount[ctxTransfer]--;
        if(SSLCTXCount[ctxTransfer] == 0) {
            //This transfer was the last one using this context, we can free it,
            //only if the current pImpl->ctx is not a ctx that could be use to generate a new Session
            SSL_CTX * currentImplCtx;
            {
                XrdSysRWLockHelper lockCrlMutex(pImpl->crlMutex);
                currentImplCtx = pImpl->ctx;
            }
            if(ctxTransfer != currentImplCtx) {
                SSL_CTX_free(ctxTransfer);
                SSLCTXCount.erase(ctxTransfer);
            }
        }
    }
}
