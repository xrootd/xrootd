...
bool TPCRequestManager::TPCQueue::TPCWorker::RunCurl(
	...
	auto curl_cleanup = [&]() {
	fprintf(stderr, "TPCWorker: Remove curl handle %s \n", request.GetIdentifier().c_str());
    ...
    do {
        ...
        if (!request.IsActive()) {
			fprintf(stderr, "TPCWorker: Not Active: Initiating cancel: %s \n", request.GetIdentifier().c_str());
			curl_cleanup();
            std::stringstream ss;
            ss << "Transfer cancelled " << request.GetIdentifier();
    ...
    if (!cancelled && res == static_cast<CURLcode>(-1)) {
		curl_cleanup();
        std::stringstream ss;
