...
TPCRequestManager::TPCQueue::TPCWorker::TPCWorker(
    const std::string &label, int scitag,  TPCQueue &queue)
    : m_label(label),
      m_queue(queue),
      m_scitag(scitag), //use this to generate the queue identifier: queues will be based on the scitag
...
bool TPCRequestManager::TPCQueue::TPCWorker::RunCurl(
	...
	auto fail_and_exit = [&](int code, const std::string& msg) -> bool {
      ...
      m_queue.m_parent.m_log.Log(
          code >= 500 ? LogMask::Error : LogMask::Info, "TPCWorker", msg.c_str());
...
void TPCRequestManager::TPCQueue::TPCWorker::Run() {
    ...
    while (true) {
    		fprintf(stderr,
        		"Consuming Queue: [tid: %lu] rahul m_alive=%d, label = %s\n",
...
int TPCRequestManager::TPCQueue::TPCWorker::opensocket_callback(
  ...
  // Return a socket file descriptor (note the clo_exec flag will be set).
	int fd =
		XrdSysFD_Socket(address->family, address->socktype, address->protocol);
	...
	if (purpose == CURLSOCKTYPE_IPCXN && clientp) {
		...
		if (!tpcWorker->m_pmark_manager.connect(fd, &(address->addr), address->addrlen,
...
int TPCRequestManager::TPCQueue::TPCWorker::closesocket_callback(
...
bool TPCRequestManager::TPCQueue::Produce(TPCRequest &handler) {
    ...
    if (m_workers.size() < m_max_workers) {
        auto worker = std::make_unique<
            ...
            handler.GetIdentifier(), handler.GetScitag(), *this);
        std::thread t(
...
TPCRequestManager::TPCQueue::TryConsume() {
	...
	fprintf(stderr, "[%s][tid: %lu] rahul: m_ops_size = %zu m_alive:%d \n",
        m_label.c_str(),
        std::hash<std::thread::id>{}(std::this_thread::get_id()),
        m_ops.size(),
		m_alive);
...
TPCRequestManager::TPCQueue::ConsumeUntil(
	...
	fprintf(stderr, "[%s][tid: %lu] rahul: m_ops_size = %zu\n",
        m_label.c_str(),
        std::hash<std::thread::id>{}(std::this_thread::get_id()),
        m_ops.size());
