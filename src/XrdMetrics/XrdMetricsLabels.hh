...
public:
         LabelSchema() = default;
explicit LabelSchema(std::vector<std::string> names) : names_(std::move(names)) {}
...
bool operator==(const LabelValues& o) const noexcept { return v == o.v; }
};
...
SeriesLabels(const LabelContext& ctx, const std::string& metricName,
             LabelValues vals);

//! Bytes that precede the value in the text exposition format.
const std::string& prometheusPrefix() const noexcept { return prefix_; }

//! This series' variable label values, in schema order.
const LabelValues& values() const noexcept { return values_; }
...
{
   if (ctx_->global) for (auto& kv : *ctx_->global) f(kv.first, kv.second);
   for (auto& kv : ctx_->constLabels)               f(kv.first, kv.second);
   const auto& n = ctx_->schema.names();
   for (std::size_t i = 0; i < n.size(); ++i)        f(n[i], values_.v[i]);
}
