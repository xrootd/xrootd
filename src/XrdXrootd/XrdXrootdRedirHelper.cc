...
namespace
{
   ...
   // ==========================================================================
   //
   // Test-only clock seam.  Production code leaves gNow == nullptr, in which
   // case Now() falls back to time(nullptr) on every call.  The pointer is
   // installed exclusively by XrdXrootdRedirHelper::SetClockForTesting() so
   // unit tests can fast-forward past gIPHold and exercise the cache-refresh
   // branch in LookupTarget.
   //
   ...
   struct netInfo
   {
      ...
      explicit netInfo(const char *id) : netID(strdup(id)) {}
   ...
   netInfo *LookupTarget(const char *netID, int port)
   {
      ...
      static std::map<const char*, netInfo*, decltype(cmpchr)> niMap(cmpchr);
...
void XrdXrootdRedirHelper::SetClockForTesting(time_t (*nowFn)())
{
   gNow = nowFn;
}
                               ...
                               std::string &outTarget, std::string &errMsg)
{
   ...
   if (port >= 0)
      {// Host[?cgi] form: split the target and invoke the plugin's host+port
       ...
       // Unchanged so the caller emits the original target unmodified.
       std::string urlHead, host, urlPort, urlTail;
                                    ...
                                    std::string &urlTail)
{
   ...
   if (!tail) {urlTail.clear(); host = hBeg;}
