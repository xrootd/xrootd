...
{
    const auto failure = [&](uint16_t code, uint32_t error,
    ...
    try
    {
        ...
        if(!status.IsOK())
        {
            return failure(status.code, status.errNo,
    ...
    {
        return failure(XrdCl::errInternal, EIO, ex.what());
    }
}
...
CurlTapeOp::Success()
{
    ...
    {
        Fail(XrdCl::errInternal, EIO, ex.what());
    }
}
...
CurlTapeOp::Write(char *buffer, size_t size)
{
    ...
    catch(const std::exception &ex)
    {
        return FailCallback(kXR_ServerError,
            std::string("while buffering a Tape REST response: ") + ex.what());
    }
    catch(...)
    {
        return FailCallback(kXR_ServerError,
