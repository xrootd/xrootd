...
{
    const auto to_ascii_lower = [](const unsigned char c) {
        return (c >= 'A' && c <= 'Z') ? static_cast<unsigned char>(c - 'A' + 'a') : c;
    };
    
    return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end(),
        [to_ascii_lower](const unsigned char a, const unsigned char b) {
...
Build(const std::string_view spec, HeaderList &headers)
{
    ...
    for (auto pos = spec.begin(); pos != spec.end(); ) {
        ...
        if (colon == std::string_view::npos) {
            log->Error(kLogXrdClHttp, "Requested header %s holds no colon; each header must read \"<name>: <value>\"",
                obfuscateAuth(std::string(entry)).c_str());
            usable = false;
            continue;
        }
        ...
        if (name.empty()) {
            log->Error(kLogXrdClHttp, "Requested header holds no name before its colon");
            usable = false;
            continue;
        }
        ...
        {
            log->Error(kLogXrdClHttp, "Requested header %s holds a name that is not an HTTP token", name.c_str());
            usable = false;
            continue;
        }
        if (IsForbiddenHeader(name)) {
            log->Error(kLogXrdClHttp, "Requested header %s must not be set by the client", name.c_str());
            usable = false;
            continue;
        }
        ...
        if (value.empty()) {
            log->Error(kLogXrdClHttp, "Requested header %s holds no value", name.c_str());
            usable = false;
            continue;
        }
        if (value.find('\r') != std::string_view::npos) {
            log->Error(kLogXrdClHttp, "Requested header %s holds a carriage return in its value", name.c_str());
            usable = false;
            continue;
        }
