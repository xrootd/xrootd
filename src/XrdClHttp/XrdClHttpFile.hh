...
#ifndef XRDCLHTTP_CURLFILE_HH
#define XRDCLHTTP_CURLFILE_HH
...
private:
    ...
    //
    // The `XrdClHttpQueryParam` property allows for additional query parameters to be added by
    ...
    std::shared_ptr<XrdClHttp::CurlPutOp> m_put_op;
    ...
    private:
        bool m_active{true};
        bool m_initial{true};
        std::shared_ptr<XrdClHttp::CurlPutOp> m_op;
    ...
    // of standalone reads.
    std::shared_ptr<XrdClHttp::CurlReadOp> m_prefetch_op;
    ...
    std::atomic<XrdClHttp::HeaderCallout *> m_header_callout{nullptr};

    // Class for setting up the required HTTP headers for S3 requests
    class HeaderCallout : public XrdClHttp::HeaderCallout {
    public:
