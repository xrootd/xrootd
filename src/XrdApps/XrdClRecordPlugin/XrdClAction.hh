...
namespace XrdCl {
  ...
  struct Action
  {
    ...
    //--------------------------------------------------------------------------
    inline void RecordResult( XRootDStatus *st,
                              AnyObject    *rsp )
    {
    ...
    inline std::string ToString()
    {
      ...
      ss << id << ',';
      ss << Name()   << ',';
      ss << std::chrono::duration_cast<std::chrono::seconds>( start.time_since_epoch() ).count() << ',';
      std::string argstr = ArgStr();
      if( !argstr.empty() )
        argstr += ';';
      ss << argstr << timeout << ',';
      ss << std::chrono::duration_cast<std::chrono::seconds>( stop.time_since_epoch() ).count() << ',';
