						...
						>> [=](const XrdCl::XRootDStatus &st, XrdCl::VectorReadInfo ch) mutable
						{
								...
								while(it!=requestedChunks.end())
								{
									...
									if(host == i)
									{
										...
										if(!st.IsOK())
										{
											log->Dump(XrdCl::XRootDMsg, "EC Vector Read of host %llu failed entirely.", (unsigned long long) i);
											...
											if( orgcksum != cksum )
											{
												log->Dump(XrdCl::XRootDMsg, "EC Vector Read: Wrong checksum for block %llu stripe %llu.", (unsigned long long) blkid, (unsigned long long) strpid);
												MissingVectorRead(currentBlock, blkid, strpid, timeout);
												continue;
											}
											else{
												...
												if(!recoverable)
													log->Dump(XrdCl::XRootDMsg, "EC Vector Read: Couldn't recover block %llu.", (unsigned long long) blkid);
											}
										}
									}
								}
		  ...
		  for(size_t index = 0; index < chunks.size(); index++){
			  ...
			  while(remainLength > 0){
			      ...
			      if(blockMap.find(blkid) == blockMap.end() || blockMap[blkid] == nullptr){
			    	  log->Dump(XrdCl::XRootDMsg, "EC Vector Read: Missing block %llu.", (unsigned long long) blkid);
			    	  failed = true;
			    	  break;
			      }
			      if(blockMap[blkid]->state[strpid] != block_t::Valid){
			    	  log->Dump(XrdCl::XRootDMsg, "EC Vector Read: Invalid stripe in block %llu stripe %llu.", (unsigned long long) blkid, (unsigned long long) strpid);
			    	  failed = true;
			    	  break;
			      }
