...
struct XrdOfsPrepPersist::Impl {
  ...
  std::thread worker;
  std::filesystem::recursive_directory_iterator sweep;
  Clock::time_point nextSweep = Clock::now();

  Impl(const std::string &root, const std::string &id, XrdOfsPrepare &b,
       XrdOucEnv *env, XrdSysError *logger, size_t maximum, uint64_t keep)
    : store(root), identity(id), backend(b), environment(env), log(logger),
        ...
        for (auto &local : record["files"]) if (local.at("path") == file.at("path")) {
          ...
          if (!Terminal(local.at("state").get<std::string>())) {
            ...
            if (Terminal(local.at("state").get<std::string>()) && !local.contains("finishedAt"))
              local["finishedAt"] = XrdOfsPrepStore::Now();
          }
        }
      }
  ...
  void Scan(bool recovery) {
    ...
    if (sweep == fs::recursive_directory_iterator()) {
      if (!recovery && Clock::now() < nextSweep) return;
      sweep = fs::recursive_directory_iterator(store.Root() / "requests");
    }
    unsigned budget = 64;
    while (sweep != fs::recursive_directory_iterator() && budget-- && !stopping) {
      const auto path = sweep->path();
      const bool regular = sweep->is_regular_file();
      ++sweep;
      if (!regular || path.extension() != ".json") continue;
      const auto id = path.stem().string();
      if (!IsId(id)) continue;
      try {
        std::lock_guard<std::mutex> guard(Lock(id));
        auto record = Load(id, false);
        if (!Finished(record)) { if (recovery) Schedule(id); }
        else if (record.contains("completedAt") &&
                 XrdOfsPrepStore::Now() > record.at("completedAt").get<uint64_t>() + retention)
          store.Erase(id);
...
XrdOfsPrepPersist::XrdOfsPrepPersist(const std::string &root, const std::string &identity,
  ...
  : m_impl(new Impl(root, identity, backend, environment, log, maxRequests, retention)) {}
