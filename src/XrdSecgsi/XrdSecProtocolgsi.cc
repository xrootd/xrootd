                                     ...
                                     const struct sockaddr *ipadd)
{
   ...
   if (hname) {
      ...
      PRINT("WARNING: host name undefined: failure may follow");
   }
#else
   if (ipadd) {
                                    ...
                                    String &emsg)
{
   ...
   if (!ServerCertNameOK(hs->Chain->End()->Subject())) {
      emsg = "server certificate subject name does not match the expected format";
      return -1;
   }
   //
