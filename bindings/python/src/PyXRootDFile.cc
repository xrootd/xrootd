...
namespace PyXRootD
{
  ...
  static PyMethodDef FileMethods[] =
  {
    ...
    { "setopenfiletemplate",
       (PyCFunction) PyXRootD::File::SetOpenFileTemplate, METH_VARARGS | METH_KEYWORDS, NULL },
    { "clone",
  ...
  //----------------------------------------------------------------------------
  //! Set the template file object to be used for samefs or clone on open
  //----------------------------------------------------------------------------
  PyObject* File::SetOpenFileTemplate( File *self, PyObject *args, PyObject *kwds )
  {
    static const char  *kwlist[] = { "src_file", "xdup", NULL };
    ...
    if ( !PyArg_ParseTupleAndKeywords( args, kwds, "O!p|:setopenfiletemplate",
         (char**) kwlist, &FileType, &tfile, &xdup ) ) return NULL;

    if ( !tfile || tfile == Py_None ) {
      PyErr_SetString( PyExc_TypeError, "setopenfiletemplate() expects a file as argument" );
      return NULL;
    }

    File *fp = reinterpret_cast<File*>(tfile);
    status = self->file->SetOpenFileTemplate(*fp->file, xdup);

    PyObject *pystatus = ConvertType<XrdCl::XRootDStatus>( &status );
    PyObject *o = Py_BuildValue( "ON", pystatus, Py_BuildValue( "" ) );
    Py_DECREF( pystatus );
    return o;
  }
