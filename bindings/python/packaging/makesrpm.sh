#!/bin/bash
#-------------------------------------------------------------------------------
# Create a source RPM package - adapted from xrootd
# Author: Lukasz Janyst <ljanyst@cern.ch> (04.04.2014)
#-------------------------------------------------------------------------------
...
VERSION=`$SOURCEPATH/genversion.sh $SOURCEPATH 2>/dev/null`
if test $? -ne 0; then
...
TEMPDIR=`mktemp -d /tmp/xrootd-python.srpm.XXXXXXXXXX`
...
#-------------------------------------------------------------------------------
if test ! -r rhel/xrootd-python.spec.in; then
  echo "[!] The specfile template does not exist!" 1>&2
  exit 7
fi
cat rhel/xrootd-python.spec.in | sed "s/__VERSION__/$VERSION/" | \
  sed "s/__RELEASE__/$RELEASE/" > $TEMPDIR/xrootd-python.spec
...
git archive --prefix=xrootd-python/ --format=tar $COMMIT | gzip -9fn > \
      $RPMSOURCES/xrootd-python.tar.gz
...
#-------------------------------------------------------------------------------
OTHER_VERSIONS=`cat $TEMPDIR/xrootd-python.spec | \
    egrep '^Source[0-9]+:[[:space:]]*xrootd-python-[0-9]+\.[0-9]+\.[0-9]+\.tar\.gz$' |\
    awk  '{ print $2; }'`

for VER in $OTHER_VERSIONS; do
    VER=${VER/xrootd-python-/}
    VER=${VER/.tar.gz/}

    git archive --prefix=xrootd-python-$VER/ --format=tar v$VER | gzip -9fn > \
        $RPMSOURCES/xrootd-python-$VER.tar.gz
...
rpmbuild --define "_topdir $TEMPDIR/rpmbuild"    \
  ...
  -bs $TEMPDIR/xrootd-python.spec > $TEMPDIR/log
if test $? -ne 0; then
