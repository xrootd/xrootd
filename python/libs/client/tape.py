...
class TapeClient(object):
  ...
  def _filesystem_url(self, url):
    ...
    if scheme in ('root', 'roots', 'xroot', 'xroots'):
      if parsed.port and parsed.port != 1094 and parsed.hostname:
        return 'https://%s:%d' % (parsed.hostname, parsed.port)
      if parsed.hostname:
  ...
  def _stage_entry(self, entry, disk_lifetime=None, targeted_metadata=None):
    ...
    if self._is_url(entry):
      payload['url'] = self._http_url(entry)
    else:
      payload['path'] = entry
  ...
  def _normalize_stage_files(self, files, disk_lifetime=None,
    ...
    for item in files:
      if isinstance(item, string_types):
        ...
        item = self._http_url(item) if self._is_url(item) else item
      ...
      entry = self._http_url(entry) if self._is_url(entry) else entry
  ...
  def stage_cancel(self, url, request_id, paths):
    ...
    status, _ = self._filesystem(url).prepare(
      [request_id] + [self._http_url(path) if self._is_url(path) else path
                      for path in paths], PrepareFlags.CANCEL,
      timeout=self.timeout)
    return status
  ...
  def release(self, url, request_id, paths):
    ...
    status, _ = self._filesystem(url).prepare(
      [request_id] + [self._http_url(path) if self._is_url(path) else path
                      for path in paths], PrepareFlags.EVICT,
      timeout=self.timeout)
    return status
  ...
  def archive_info(self, urls):
    ...
    http_urls = [self._http_url(url) for url in urls]
