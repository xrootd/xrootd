...
def test_stage_rejects_invalid_targeted_metadata():
  ...
  with pytest.raises(ValueError):
    client.stage('root://xrootd.example.org/store/file', [
      {'path': '/store/file', 'targeted_metadata': ['analysis']},
    ])
...
def test_native_xrootd_urls_preserve_protocol(scheme):
  client = tape.TapeClient()
  status, response = client.stage(
  ...
  assert status.ok
  assert response.request_id == 'request-1'
  expected_endpoint = '%s://xrootd.example.org:1094' % scheme
  assert FakeFileSystem.instances[0].url == expected_endpoint
  assert FakeFileSystem.instances[0].calls == [
    ('prepare', ['%s://xrootd.example.org:1094/store/file' % scheme],
     PrepareFlags.STAGE, 0, 0),
  ]


def test_native_xrootd_failure_does_not_retry_with_https(monkeypatch):
  class FailingFileSystem(FakeFileSystem):
    def prepare(self, files, flags, priority=0, timeout=0):
      self.calls.append(('prepare', files, flags, priority, timeout))
      return _status(False), ''

  monkeypatch.setattr(tape, 'FileSystem', FailingFileSystem)
  client = tape.TapeClient()
  status, response = client.stage(
  ...
  assert response == ''
  assert len(FakeFileSystem.instances) == 1
  assert FakeFileSystem.instances[0].url == 'root://xrootd.example.org:8444'
  assert FakeFileSystem.instances[0].calls == [
    ('prepare', ['root://xrootd.example.org:8444/store/file'],
     PrepareFlags.STAGE, 0, 0),
  ]
